...
void Matrix<T>::SetRows(const int rows) {
  ...
  if (rows != this->rows_) this->ResizeMatrix(rows, this->cols_);
}

template <typename T>
void Matrix<T>::SetCols(const int cols) {
  if (cols < 1) throw std::out_of_range("Error: Wrong cols size");
  if (cols != this->cols_) this->ResizeMatrix(this->rows_, cols);
}
...
T& Matrix<T>::operator()(int row, int col) {
  if (row >= rows_ || col >= cols_ || row < 0 || col < 0)
    throw std::range_error(
        "Incorrect matrix size: row = " + std::to_string(row) +
        " col = " + std::to_string(col) + " size = " + std::to_string(rows_) +

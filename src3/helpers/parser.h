...
class Parser {
  private:
    ...
    std::vector<double> ParsingLineForWeights_(std::string line);

public:
    Parser() : error_(false) {}
    ~Parser() {}
    using Dataset = std::vector<std::vector<int>>;
    using Topology = std::vector<std::vector<std::vector<double>>>;
//    using Topology = std::vector<std::pair<int, int>>;

    int count_string = 0;
//    int count_elemets_char = 0;

    Dataset Parsing(const std::string &file_name);
    Topology ParsingWeights(const std::string &file_name);
    void Print();
    Dataset data_set_;
    Topology  topology_;

...
s21::Network::Network(std::string path, size_t output_size, double learning_rate) :
m_learning_rate_(learning_rate) {
  s21::Parser parser;
  std::vector<std::vector<std::vector<double>>> topology = parser.ParsingWeights(path);
  for (size_t i = 0; i < topology.size(); ++i) {
    m_layers_.push_back(new Layer(topology[i]));
  }
  m_layers_.push_back(new Layer(m_layers_.at(m_layers_.size() - 1)->GetNeurons().size(), output_size));
}
...
//  }
  ...
  for (size_t i = 0; i < m_layers_.back()->GetNeurons().size(); ++i) {
    std::cout << m_layers_.back()->GetNeurons().at(i)->GetValue() << " ";
  }
...
//  передать параметром имя файл из интерфейса
void s21::Network::save() {
  std::ofstream out("weight.txt", std::ios::out | std::ios::trunc);
  if (out.is_open()) {
    out << m_layers_.size() << "\n";
    for (size_t i = 0; i < m_layers_.size(); ++i) {
      for (Neuron* n : m_layers_.at(i)->GetNeurons()) {
